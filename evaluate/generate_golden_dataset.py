...
from app.weaviate_loader import get_available_sources, load_all_chunks_from_weaviate  # noqa: E402
from evaluate.llm_as_judge import LLMAsJudgePipeline  # noqa: E402
...
範例用法:
  ...
  python generate_golden_dataset.py --source weaviate --generator qwen2:7b --evaluator llama3:8b --count 50
    ...
    input_group.add_argument("--input", "-i", type=str, help="輸入的 chunks pickle 檔案路徑")

    input_group.add_argument(
        "--source", "-s", type=str, help='從 Weaviate 載入 chunks（"weaviate" = 全部，或指定特定來源名稱）'
    )

    input_group.add_argument("--list-sources", action="store_true", help="列出 Weaviate 中所有可用的來源並退出")
    ...
    parser.add_argument("--count", "-c", type=int, default=None, help="想要生成的 QA 對數量（預設: 處理全部 chunks）")
    ...
    if args.list_sources:
        ...
        try:
            ...
            if sources:
                print(f"\n✓ 找到 {len(sources)} 個來源")
                print("\n使用範例：")
                print("  python generate_golden_dataset.py --source weaviate --count 50")
    ...
    try:
        results = pipeline.process_chunks(chunks=chunks, max_qa_pairs=args.count, save_to_file=args.output)
