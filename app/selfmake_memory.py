...
class CustomMemory:
    ...
    def _save_to_weaviate(self, message):
        ...
        try:
            ...
            self.weaviate_client.data_object.create(
                data_obj, class_name="ChatHistory", uuid=message["uuid"], vector=embedding
            )
        except Exception as e:
    ...
    def search_relevant_history(self, query, top_k=3):
        ...
        try:
            ...
            result = (
                self.weaviate_client.query.get("ChatHistory", ["role", "content", "timestamp", "sources"])
            ...
            history = []
            for item in data:
                msg = {"role": item["role"], "content": item["content"], "timestamp": item["timestamp"]}
                if item.get("sources"):
                    try:
