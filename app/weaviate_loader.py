...
def load_all_chunks_from_weaviate(source_filter: str | None = None, limit: int = 10000) -> list[dict[str, str]]:
    """
    ...
    print("📥 載入 chunks from Weaviate...")

    # 構建查詢
    query_obj = weaviate_client.query.get("Paragraph", ["text", "source"]).with_limit(limit)
    ...
    try:
        ...
        chunks = [
            {"text": item["text"], "source": item.get("source", "Unknown")}
...
def get_available_sources() -> list[str]:
    ...
    try:
        ...
        result = (
            weaviate_client.query.aggregate("Paragraph")
        ...
        sources = [group["groupedBy"]["value"] for group in result["data"]["Aggregate"]["Paragraph"]]
...
if __name__ == "__main__":
    ...
    if sources:
        ...
        print("-" * 60)
        filtered_chunks = load_all_chunks_from_weaviate(source_filter=sources[0], limit=5)
