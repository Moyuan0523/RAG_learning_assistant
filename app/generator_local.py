...
def generate_answer_with_transformers(query: str, contexts: list[str], history: list = []):
    ...
    if not hasattr(generate_answer_with_transformers, "model"):
        ...
        model = AutoModelForCausalLM.from_pretrained(LOCAL_LLM_MODEL, torch_dtype=torch.float16, device_map="auto")
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs, max_new_tokens=1024, temperature=0.3, do_sample=True, pad_token_id=tokenizer.eos_token_id
        )
...
def generate_answer(query: str, contexts: list[str], history: list = [], model: str = None) -> str:
    """
    生成回答的主要函數
        ...
        if USE_LOCAL_LLM and OPENAI_API_KEY:
            ...
            try:
                ...
                response = backup_client.chat.completions.create(
                    model="gpt-3.5-turbo", messages=messages, temperature=0.3, max_tokens=1024
                )
                return response.choices[0].message.content.strip()
