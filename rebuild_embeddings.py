...
from app.gen_chunks_Index_to_weaviate import connect_weaviate, pdf_to_weaviate  # noqa: E402
...
def rebuild_all_pdfs(sources_folder: str = "Sources"):
    ...
    for i, pdf_path in enumerate(pdf_files, 1):
        ...
        try:
            pdf_to_weaviate(pdf_filename=pdf_path.name, upload_folder=sources_folder)
...
def rebuild_single_pdf(pdf_filename: str, sources_folder: str = "Sources"):
    ...
    try:
        pdf_to_weaviate(pdf_filename=pdf_filename, upload_folder=sources_folder)
...
範例:
  ...
  python rebuild_embeddings.py --no-clear
    ...
    parser.add_argument("--pdf", type=str, help="只處理指定的 PDF 檔案")

    parser.add_argument("--sources-folder", type=str, default="Sources", help="PDF 檔案所在的資料夾（預設: Sources）")

    parser.add_argument("--no-clear", action="store_true", help="不清空 Weaviate 中的舊數據（增量添加）")
    ...
    print("🤖 Embedding 模型: paraphrase-multilingual-MiniLM-L12-v2")
    print("=" * 70)
